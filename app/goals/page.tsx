    ...
    <main style={{ padding: 16, maxWidth: 600, margin: '0 auto' }}>
      ...
      {goals.length === 0 && !showCreateForm && (
        <div className="card animate-in" style={{ textAlign: 'center', padding: '40px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
          ...
          <h3 style={{ margin: 0, color: 'var(--text)', fontFamily: 'var(--font-display)' }}>Nenhuma meta ainda</h3>
          <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: 14 }}>Crie sua primeira meta individual ou compartilhada com o casal.</p>
          <button onClick={() => { setShowCreateForm(true); setErrorMsg(null); }} style={{ marginTop: 8, padding: '10px 20px', backgroundColor: 'var(--blue)', color: 'white', border: 'none', borderRadius: 'var(--radius-sm)', cursor: 'pointer', fontWeight: 600, fontSize: 14, boxShadow: '0 4px 12px rgba(108,99,255,0.3)' }}>
            ➕ Criar primeira meta
          </button>
        </div>
      )}
